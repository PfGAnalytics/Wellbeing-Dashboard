...
suppressWarnings({ # Turn off warnings
  ...
  # ---- 1) Replace updated.json inside quick-read.js ----
  js_code <- readLines(quick_js, warn = FALSE)

  js_code <- gsub(
    "scripts/updated.json",
    json_data_url,   # your data URL created earlier
    js_code,
    fixed = TRUE
  )

  # Write back WITHOUT adding a trailing newline
  cat(js_code, file = quick_js, sep = "\n")

  # ---- 2) Replace domains_data.js inside quick-read.js ----
  js_code <- readLines(quick_js, warn = FALSE)

  js_code <- gsub(
    "scripts/domains_data.js",
    domains_data_url,   # your data URL for domains_data.js
    js_code,
    fixed = TRUE
  )
  ...
  # ---- 3) Embed quick-read.js into the HTML ----
  index <- gsub(
